...
> 特性：以 Rust workspace 为中心的 AI Agent runtime，覆盖 thread、job、tool、MCP 与 desktop 的分层边界与协作约束。

## 作用域

- 本文件适用于仓库根目录及其所有子目录。
- 如果更深目录存在 `AGENTS.md`，以更近的文件为准。

## 先读哪里

- 根目录规则适用于整个仓库。
- 进入 `crates/*` 或更深目录时，优先遵循最近的 `AGENTS.md`。
...
| `argus-thread-pool` | 共享 runtime pool 核心 | 只负责 runtime residency、lifecycle 与 generic delivery，不感知 session/job 语义 |
| `argus-job` | 后台 job 生命周期与 runtime pool 解耦 | `JobManager` 管 job，`ThreadPool` 只管 runtime 生命周期 |
...
- crate 级 `AGENTS.md` 顶部保留一句话 `> 特性：...`
- 新增或调整 crate 文档时，保持 `AGENTS.md` 为该目录的唯一 guidance 入口
