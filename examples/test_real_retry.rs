...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let base_url = std::env::var("ARGUS_LLM_BASE_URL").unwrap_or_else(|_| "https://open.bigmodel.cn/api/paas/v4".to_string());
    let api_key = std::env::var("ARGUS_LLM_API_KEY").unwrap_or_else(|_| "fb7e67c50762405c8a8e134449af6442.FcifS4oZxuQIkCvQ".to_string());
    ...
    let retry_provider = Arc::new(RetryProvider::new(test_provider, RetryConfig { max_retries: 3 }));
    ...
    for i in 1..=5 {
        ...
        let request = CompletionRequest::new(vec![
            ChatMessage::user(format!("Reply with exactly 'Call {} completed'", i))
        ]);
