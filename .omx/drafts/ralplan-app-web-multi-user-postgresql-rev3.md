...
- Postgres-backed server smoke tests cover `/api/v1/account`, `/api/v1/bootstrap`, providers, templates, MCP server listing, and agent runs in addition to chat isolation, proving shared/global server surfaces still work on Postgres.
...
- Postgres server smoke tests cover non-chat shared/global endpoints: account status/configuration, bootstrap, providers/templates, MCP listing, tools/runtime where feasible, and agent runs.
