...
Implement a first-pass multi-user `apps/web` + `argus-server` deployment backed by PostgreSQL. The only user-isolated product data in this pass is chat sessions, threads, and messages. Current chat user identity comes from trusted request headers now; later OAuth2 should populate the same server-side user context. The identity contract is explicit: `X-Argus-User-Id` is an external stable string, while persisted `users.id` is an internal UUID; the Postgres `users` table stores `external_id TEXT UNIQUE NOT NULL` plus optional display name, and all chat ownership FKs reference the internal UUID. OAuth2, temporary login, frontend user switcher, workspace/team model, user-scoped providers/templates/MCP/jobs/runtime, SQLite data migration, and SQLite runtime support for server/web are out of scope.
...
Steps:
1. Add a strongly typed internal `UserId` shared boundary type backed by UUID for persisted ownership. Add a separate validated `ExternalUserId`/header string in `argus-server` for `X-Argus-User-Id`; repository upserts/maps `external_id` to internal `users.id`.
2. Keep `RequestUser` and trusted-header extraction in `argus-server`; do not place HTTP/header semantics in `argus-protocol`. `RequestUser` may carry both `external_id` and resolved internal `UserId` after server/repository resolution.
...
5. Refactor `ServerCore` assembly so server uses `ArgusPostgres` for every repository trait still assembled for server-visible endpoints: `AccountRepository`, `LlmProviderRepository`, `AgentRepository`, `JobRepository`, `McpRepository`, `AgentRunRepository`, `SessionRepository`, and `ThreadRepository`. Shared/global surfaces remain operational on Postgres; they are not removed or user-scoped. Avoid broad generic repository injection unless necessary; assembly remains centralized in `ServerCore`.
...
- Mandatory Postgres-backed non-chat server API verification matrix must run on the Postgres harness: health (optional/non-repo-backed smoke), account get/configure, bootstrap, providers CRUD/test, templates CRUD, MCP CRUD/test/tool-discovery, tools registry, runtime snapshot/events where testable without live long-running runtime, and agent runs create/get. These prove shared/global server surfaces still work on Postgres in addition to chat isolation.
...
Steps:
1. Add `users` table for header-derived chat ownership only. It is not the `accounts` auth table. Persist `users.id UUID PRIMARY KEY`, `external_id TEXT UNIQUE NOT NULL`, optional display name, and timestamps; trusted header values map through `external_id` to internal UUID ownership.
...
- User identity mapping tests prove `X-Argus-User-Id` external strings upsert/map to stable internal UUID `users.id` records and never reuse `accounts`.
...
- Mandatory Postgres non-chat server API suite covers: account status/configuration, bootstrap, providers CRUD/test, templates CRUD, MCP CRUD/test/tool-discovery, tools registry, runtime snapshot/events where testable without live long-running runtime, and agent runs create/get. Health can remain a lightweight optional smoke because it is not repository-backed.
...
## Task 3 Review Addendum — Documentation and Quality Gates

This addendum converts the plan into review gates for the implementation lanes. It is intentionally documentation-only: the Task 3 review worktree still shows the pre-implementation baseline, so the review finding is that later source changes must be validated against the gates below before the overall change can be accepted.

### Baseline Review Evidence

- `crates/argus-server/src/server_core.rs` still imports `ArgusSqlite`, SQLite connect/migrate helpers, and `SqlitePool`; no `ArgusPostgres` server assembly is present in this baseline.
- `crates/argus-server/src/db.rs` still accepts `sqlite:` URLs and filesystem paths and defaults `DATABASE_URL` to `~/.arguswing/sqlite.db`; this must change for the server/web runtime lane.
- `crates/argus-server/src/routes/chat.rs` chat handlers take `State`, `Path`, and `Json` extractors only; no fail-closed `RequestUser` extractor is present in this baseline.
- `crates/argus-session/src/manager.rs` chat APIs such as `load`, `send_message`, `get_thread_messages`, and `subscribe` still operate by `SessionId`/`ThreadId` alone; cache and runtime access must become user-safe before any chat route is considered isolated.
- `apps/web/src/lib/api.ts` still constructs native `EventSource` for chat SSE; trusted user headers cannot be supplied by browser JavaScript for production SSE and must be handled by gateway/proxy or a documented test harness.

### Lane Review Boundaries

| Lane | Owned boundary | Must preserve | Completion blocker |
| --- | --- | --- | --- |
| Repository/PostgreSQL | `argus-repository` traits, Postgres module, migrations, sqlx features | SQLite module and migrations continue compiling for non-server consumers | Any chat-facing repository method can read/write by `SessionId` or `ThreadId` without `UserId` ownership predicates. |
| Server/user context | `argus-server` DB target, `RequestUser` extractor, routes, `ServerCore` assembly | Route paths stay stable; `/api/v1/account` remains separate from chat users | Missing/malformed `X-Argus-User-Id` reaches chat business logic or maps to a default user. |
| Session isolation | `argus-session` manager cache/load/subscribe/trace/runtime paths | Provider/template resolution remains global during thread creation | Cached session/runtime/SSE subscription can be returned before repository ownership validation. |
| Web/header behavior | `apps/web` API helpers and deployment docs | No login UI or user switcher in this pass | Documentation claims browser `EventSource` can attach trusted headers directly. |
| Compile coexistence | `argus-wing`, `argus-template`, SQLite-dependent non-server crates | `ArgusSqlite` remains available outside server runtime | Server Postgres work deletes or renames SQLite APIs required by non-server consumers. |
| Security/build verification | Tests, architecture checks, release notes | SQL stays in repository; secrets and message content stay out of logs | Completion lacks Postgres-backed chat and non-chat server API evidence. |

### Code Quality Review Checklist

Reviewers must check these invariants directly in source, not only through passing tests:

1. **Identity model separation:** `X-Argus-User-Id` remains an external stable string, maps through `users.external_id`, and persists ownership only through internal UUID `users.id`. The existing `accounts` table and `AccountManager` are not reused as chat user identity.
2. **Fail-closed extraction:** every chat route receives a validated `RequestUser`; missing, empty, malformed, or untrusted headers fail before calling `ServerCore` or `SessionManager`.
3. **Repository-owned isolation:** every session/thread/message Postgres query includes user ownership in the repository layer. Upper layers may pass context, but must not implement SQL filtering themselves.
4. **User-safe cache/runtime access:** `SessionManager` cannot return cached sessions, register runtimes, recover traces, enqueue messages, cancel threads, read snapshots/messages, or subscribe to SSE by `SessionId`/`ThreadId` alone.
5. **SSE denial symmetry:** cross-user SSE subscription is denied with the same existence-hiding semantics as REST reads, and denial occurs before runtime load or trace recovery.
6. **Shared/global surfaces stay global:** providers, templates, MCP, jobs, runtime state, tools registry, and agent runs are Postgres-backed for server visibility but are not accidentally scoped to chat users in this first pass.
7. **Compile coexistence:** `argus-repository::sqlite`, `ArgusSqlite`, `argus-wing`, and `TemplateManager` SQLite paths continue to compile and pass targeted checks.
8. **SQL boundary:** new application SQL belongs in `crates/argus-repository`; any SQL outside the repository crate needs explicit justification in review notes.

### Mandatory Verification Matrix

Task completion must include PASS/FAIL evidence for each row. Failures may be accepted only when documented as pre-existing or environment-gated with concrete output and a follow-up owner.

| Area | Minimum command or harness | Required proof |
| --- | --- | --- |
| Repository Postgres schema | `cargo test -p argus-repository` with a PostgreSQL `DATABASE_URL`/test harness | Empty DB migrates; `users`, `sessions`, `threads`, and `messages` use UUID-compatible IDs and FK joins round trip. |
| Repository isolation | User A/B repository tests | User B cannot list/get/rename/delete/read/write user A sessions, threads, or messages. |
| Server chat REST | `cargo test -p argus-server chat_api -- --nocapture` against Postgres | All chat routes require trusted user headers; cross-user access is denied/hidden; missing headers fail closed. |
| Server chat SSE | Chat SSE integration test or equivalent harness | Cross-user SSE subscription fails before runtime load, trace recovery, or broadcast subscription. |
| Server non-chat Postgres APIs | Postgres-backed server API suite | Account status/configuration, bootstrap, providers CRUD/test, templates CRUD, MCP CRUD/test/tool discovery, tools registry, runtime snapshot/events where practical, and agent runs create/get work on Postgres. |
| Web API behavior | `cd apps/web && pnpm exec vitest run src/lib/api.test.ts src/features/chat/chat-page.test.ts src/features/chat/composables/useChatThreadStream.test.ts` | Web tests reflect the documented trusted-header path and do not rely on browser `EventSource` custom headers for production. |
| SQLite compile coexistence | `cargo test -p argus-template` and `cargo test -p argus-wing` | Non-server SQLite consumers still compile/pass after server runtime switches to Postgres. |
| SQL boundary/security | `rg -n "SELECT|INSERT|UPDATE|DELETE|sqlx::query" crates --glob '*.rs'` plus reviewer inspection | New SQL is confined to repository modules; logs avoid secrets, message content, and public trust of raw client headers. |
| Full Rust gates | `cargo test` and `cargo deny check` | Workspace regressions are absent or documented with exact unrelated failure evidence. |
| Full web gates | `cd apps/web && pnpm exec vitest run` and `cd apps/web && pnpm build` | Frontend regressions are absent or documented with exact unrelated failure evidence. |
...
- Rev3 applies Architect re-review feedback: fixes UserId contract by mapping external trusted header IDs to internal UUID `users.id`, and explicitly requires `ArgusPostgres` coverage/smoke tests for all server-visible repository surfaces assembled by `ServerCore`.
- Final applies Critic final blocker: replaces partial non-chat smoke wording with a mandatory Postgres server API verification matrix for account, bootstrap, providers, templates, MCP, tools, runtime, and agent runs.
- Task 3 review addendum documents baseline review evidence, lane boundaries, code quality gates, and the mandatory verification matrix that implementation lanes must satisfy.
