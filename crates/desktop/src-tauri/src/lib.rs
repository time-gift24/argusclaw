...
async fn query_workflow(state: State<'_, AppState>, id: String) -> Result<String, String> {
    ...
    let variables: Variables = serde_json::from_str(&format!(r#"{{"id": "{}"}}"#, id)).map_err(|e| e.to_string())?;
    let request = Request::new(query).variables(variables);
    let res = guard.execute(request).await;
    if !res.errors.is_empty() {
        return Err(res.errors.iter().map(|e| e.message.clone()).collect::<Vec<_>>().join(", "));
    }
...
async fn create_workflow(state: State<'_, AppState>, name: String) -> Result<String, String> {
    let guard = state.schema.read().await;
    let query = r#"mutation($name: String!) { createWorkflow(input: { name: $name }) { id name } }"#;
    let variables: Variables = serde_json::from_str(&format!(r#"{{"name": "{}"}}"#, name)).map_err(|e| e.to_string())?;
    let request = Request::new(query).variables(variables);
    let res = guard.execute(request).await;
    if !res.errors.is_empty() {
        return Err(res.errors.iter().map(|e| e.message.clone()).collect::<Vec<_>>().join(", "));
...
#[command]
async fn add_stage(state: State<'_, AppState>, workflow_id: String, name: String, sequence: i32) -> Result<String, String> {
    let guard = state.schema.read().await;
    ...
    let variables: Variables = serde_json::from_str(&format!(r#"{{"workflowId": "{}", "name": "{}", "sequence": {}}}"#, workflow_id, name, sequence)).map_err(|e| e.to_string())?;
    let request = Request::new(query).variables(variables);
    let res = guard.execute(request).await;
    if !res.errors.is_empty() {
        return Err(res.errors.iter().map(|e| e.message.clone()).collect::<Vec<_>>().join(", "));
...
#[command]
async fn add_job(state: State<'_, AppState>, stage_id: String, agent_id: String, name: String) -> Result<String, String> {
    let guard = state.schema.read().await;
    ...
    let variables: Variables = serde_json::from_str(&format!(r#"{{"stageId": "{}", "agentId": "{}", "name": "{}"}}"#, stage_id, agent_id, name)).map_err(|e| e.to_string())?;
    let request = Request::new(query).variables(variables);
    let res = guard.execute(request).await;
    if !res.errors.is_empty() {
        return Err(res.errors.iter().map(|e| e.message.clone()).collect::<Vec<_>>().join(", "));
...
#[command]
async fn update_job_status(state: State<'_, AppState>, job_id: String, status: String) -> Result<String, String> {
    let guard = state.schema.read().await;
    let query = r#"mutation($jobId: ID!, $status: String!) { updateJobStatus(input: { jobId: $jobId, status: $status }) { id status } }"#;
    let variables: Variables = serde_json::from_str(&format!(r#"{{"jobId": "{}", "status": "{}"}}"#, job_id, status)).map_err(|e| e.to_string())?;
    let request = Request::new(query).variables(variables);
    let res = guard.execute(request).await;
    if !res.errors.is_empty() {
        return Err(res.errors.iter().map(|e| e.message.clone()).collect::<Vec<_>>().join(", "));
...
pub fn run() {
    ...
    let schema = tokio::runtime::Handle::current()
        .block_on(async {
            ...
            // Run migrations
            sqlite::migrate(&pool).await.expect("Failed to run migrations");
