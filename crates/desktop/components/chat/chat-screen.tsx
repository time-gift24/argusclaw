...
export function ChatScreen() {
  ...
  const setSelectedProviderPreference = useChatStore((state) => state.selectProviderPreference);
  ...
  React.useEffect(() => {
    ...
    // Switch both template and provider, then activate
    setSelectedTemplateId(session.template_id ?? null);
    setSelectedProviderPreference(session.provider_id ?? null);
    void activateSession(session.template_id ?? 0);
  }, [activeSessionId, sessions, activateSession, setSelectedTemplateId, setSelectedProviderPreference]);
