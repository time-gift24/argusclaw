...
export function ThreadSidebar() {
  const { sessions, activeSessionId, fetchSessions, deleteSession, updateTitle, selectSession, cleanup } =
    useThreadListStore();
  const { activateSession, selectedTemplateId } = useChatStore();
  ...
  const handleCreateThread = async () => {
    ...
    setIsCreating(true);
    try {
      const session = await chat.createChatSession(selectedTemplateId, null);
      await fetchSessions();
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      if (deleteDialog.sessionId === activeSessionId) {
        const remaining = sessions.filter((s) => s.id !== deleteDialog.sessionId);
