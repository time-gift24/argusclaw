...
function ContextDisplayRootBase({
  ...
  const rawReasoningTokens = usage?.reasoningTokens ?? 0;
  ...
  useEffect(() => {
    setTokenState((prev) => {
      ...
      if (rawTokens > 0 && rawTokens !== prev.totalTokens) {
        return { ...prev, totalTokens: rawTokens, reasoningTokens: rawReasoningTokens, usage };
      }
      if (usage !== prev.usage) {
        return { ...prev, usage };
      }
  ...
  }, [threadId, rawTokens, rawReasoningTokens, usage]);

  const totalTokens = tokenState.totalTokens;
  const reasoningTokens = tokenState.reasoningTokens;
  // Calculate percentage using input + output (excluding reasoning tokens)
  const effectiveTokens = inputTokens + outputTokens;
  const percent = getUsagePercent(effectiveTokens, modelContextWindow);
...
function ContextDisplayRootWithUsage({
  modelContextWindow,
  children,
  usage,
}: {
...
function ContextDisplayRoot(props: ContextDisplayRootProps) {
  // usage is ThreadTokenUsage | undefined
  // If usage is defined (truthy), use it directly without calling useThreadTokenUsage
  // If usage is undefined or null, show empty state (0% ring) without calling useThreadTokenUsage
  if (props.usage) {
    return (
      <ContextDisplayRootWithUsage
  ...
  // Show empty ring state when no usage data
  return (
    <ContextDisplayRootWithUsage
      modelContextWindow={props.modelContextWindow}
      usage={undefined}
