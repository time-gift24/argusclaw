...
const providerDialogPath = new URL("../components/settings/provider-form-dialog.tsx", import.meta.url);
...
test("agent editor treats provider as optional when deciding whether the form can save", () => {
  ...
  const providerSelectBlock = agentEditorSource.match(
    /<select[\s\S]*?id="provider_id"[\s\S]*?<\/select>/,
  )?.[0] ?? "";

  assert.match(agentEditorSource, /function getPreferredProviderId/);
  assert.match(agentEditorSource, /providersData\.find\(\(p\)\s*=>\s*p\.is_default\)\?\.id \|\| providersData\[0\]\?\.id \|\| ""/);
  assert.match(
    agentEditorSource,
    /const canSave = Boolean\(\s*formData\.id\.trim\(\)\s*&&\s*formData\.display_name\.trim\(\)\s*&&\s*formData\.system_prompt\.trim\(\),\s*\)/,
  );
  assert.match(agentEditorSource, /<Button size="sm" onClick=\{handleSave\} disabled=\{saving \|\| !canSave\}>/);
...
test("provider editing flow controls the dialog from the page state", () => {
  ...
  assert.match(providersPageSource, /onOpenChange=\{\(open\)\s*=>\s*!open\s*&&\s*setEditingProvider\(null\)\}/);
  assert.match(providerDialogSource, /open\?: boolean/);
  assert.match(providerDialogSource, /onOpenChange\?: \(open: boolean\) => void/);
  assert.match(providerDialogSource, /const handleOpenChange = React\.useCallback/);
  assert.match(providerDialogSource, /<Dialog open=\{open\} onOpenChange=\{handleOpenChange\}>/);
});
