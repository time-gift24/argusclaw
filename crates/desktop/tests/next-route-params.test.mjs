...
const editProviderPageSource = readFileSync(
  new URL("../app/settings/providers/[id]/page.tsx", import.meta.url),
  "utf8",
);
...
test("dynamic agent settings page awaits Next 16 route params", () => {
  ...
  assert.match(editAgentPageSource, /safeDecodeRouteId/);
});

test("dynamic provider settings page safely decodes route ids", () => {
  assert.match(editProviderPageSource, /async function EditProviderPage/);
  assert.match(editProviderPageSource, /params:\s*Promise<\{\s*id:\s*string\s*\}>/);
  assert.match(editProviderPageSource, /const\s*\{\s*id\s*\}\s*=\s*await params/);
  assert.match(editProviderPageSource, /safeDecodeRouteId/);
});
