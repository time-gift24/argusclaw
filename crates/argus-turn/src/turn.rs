...
use argus_protocol::{
    BeforeCallLLMContext, HookAction, HookEvent, HookHandler, ThreadEvent, TokenUsage,
    ToolHookContext,
};
...
#[builder(
    pattern = "owned",
    build_fn(error = "TurnError"),
    name = "TurnBuilder"
)]
pub struct Turn {
    /// Turn identifier (auto-generated from thread_id and turn_number).
    #[builder(default = "generate_turn_id(self.thread_id.as_ref().unwrap_or(&\"unknown\".to_string()), *self.turn_number.as_ref().unwrap_or(&0))")]
    pub id: String,
...
impl Turn {
    ...
    pub async fn execute(mut self) -> Result<TurnOutput, TurnError> {
        ...
        let _ = self
            .thread_event_tx
            .send(ThreadEvent::Idle { thread_id: self.thread_id.clone() });
        ...
        for hook in &self.hooks {
            let action = match event {
                HookEvent::BeforeCallLLM => {
                    let ctx_any = ctx
                        .downcast_ref::<BeforeCallLLMContext>()
                        .ok_or_else(|| TurnError::LlmCallBlocked {
                ...
                HookEvent::BeforeToolCall | HookEvent::AfterToolCall | HookEvent::TurnEnd => {
                    let ctx_any = ctx.downcast_ref::<ToolHookContext>().ok_or_else(|| {
                        TurnError::ToolCallBlocked {
                            reason: "Invalid hook context".to_string(),
                        }
                    })?;
            ...
            if let HookAction::Block(reason) = action {
                return match event {
                    HookEvent::BeforeCallLLM => Err(TurnError::LlmCallBlocked { reason }),
                    HookEvent::BeforeToolCall => Err(TurnError::ToolCallBlocked { reason }),
                    _ => Ok(HookAction::Block(reason)),
                };
            }
    ...
    async fn execute_loop(&mut self) -> Result<TurnOutput, TurnError> {
        ...
        for iteration in 0..max_iterations {
            ...
            match self.fire_hooks(HookEvent::BeforeCallLLM, &ctx).await? {
                HookAction::Continue => {}
            ...
            match self.process_finish_reason(response, &mut messages, &mut token_usage)? {
                NextAction::Return(output) => {
                    ...
                    let _ = self.fire_hooks(HookEvent::TurnEnd, &ctx).await;
    ...
    async fn call_llm_streaming(
        ...
        match self.provider.stream_complete_with_tools(request.clone()).await {
            Ok(mut stream) => {
                ...
                while let Some(event_result) = stream.next().await {
                    ...
                    let _ = self.stream_tx.send(TurnStreamEvent::LlmEvent(event.clone()));
    ...
    async fn execute_tools_parallel(
        ...
        let futures: Vec<_> = tool_calls
            .into_iter()
            .map(|tool_call| {
                self.execute_single_tool(tool_call, tool_timeout_secs)
            })
            .collect();
    ...
    async fn execute_single_tool(
        ...
        if let Ok(HookAction::Block(reason)) =
            self.fire_hooks(HookEvent::BeforeToolCall, &ctx).await
        {
            ...
            let _ = self.fire_hooks(HookEvent::AfterToolCall, &after_ctx).await;
        ...
        let _ = self
            .stream_tx
            .send(TurnStreamEvent::ToolCompleted {
                tool_call_id: tool_call_id.clone(),
                tool_name: tool_name.clone(),
                result: event_result,
            });
        ...
        let _ = self.fire_hooks(HookEvent::AfterToolCall, &ctx).await;
...
mod tests {
    ...
    fn test_turn_debug_format() {
        ...
        assert!(stream_tx.receiver_count() >= 0);
        assert!(thread_event_tx.receiver_count() >= 0);
    }
