...
pub fn create_zhipu_provider(config: ZhipuFactoryConfig) -> Result<Arc<dyn LlmProvider>, LlmError> {
    let provider: Arc<dyn LlmProvider> =
        Arc::new(ZhipuProvider::new(config.provider)?);
...
mod tests {
    ...
    fn zhipu_provider_reports_thinking_for_glm45() {
        let provider = ZhipuProvider::new(
            ZhipuConfig::new("test-key", "glm-4.5").with_timeout(std::time::Duration::from_secs(30)),
        )
    ...
    fn zhipu_provider_reports_thinking_for_glm5() {
        let provider = ZhipuProvider::new(ZhipuConfig::new("test-key", "glm-5")).expect("provider should build");
