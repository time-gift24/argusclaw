...
- chat / thread / message API 仅按 Phase 5 的 server-only 边界扩展；不改 desktop 主流程；thread event SSE 只允许镜像现有 `ThreadEvent`，不新增 desktop rewiring
- 不新增 settings/admin_settings 持久化；实例名作为产品展示文案由 bootstrap 返回
