...
- Phase 5A 起允许暴露窄 chat REST API：sessions / threads / messages / send / cancel
...
- chat / thread / message API 仅按 Phase 5A 的 server-only REST 边界扩展；不改 desktop 主流程，不做 web chat UI，不加 thread event SSE
