...
#[derive(Debug)]
pub(super) struct DummyProviderResolver;
...
pub(super) fn test_job_manager() -> JobManager {
...
pub(super) struct FixedProviderResolver {
    provider: Arc<dyn LlmProvider>,
}

impl FixedProviderResolver {
    pub(super) fn new(provider: Arc<dyn LlmProvider>) -> Self {
        Self { provider }
    }
}
...
impl CapturingProvider {
    pub(super) fn new(response: &str, delay: Duration, token_count: u32) -> Self {
        Self {
...
pub(super) async fn test_job_manager_with_provider(
    provider: Arc<dyn LlmProvider>,
...
pub(super) async fn test_persistent_job_manager_without_default_provider() -> JobManager {
    let pool = SqlitePool::connect("sqlite::memory:")
        .await
        .expect("sqlite memory pool should connect");
    migrate(&pool).await.expect("migration should succeed");
...
pub(super) fn assistant_output(content: &str) -> TurnRecord {
...
pub(super) fn sample_job_result(job_id: impl Into<String>) -> ThreadJobResult {
    ThreadJobResult {
        job_id: job_id.into(),
        success: true,
        cancelled: false,
