...
`agents/builtins.rs` 使用 `include_str!` 在编译期嵌入 TOML 定义的默认 agent。`load_arguswing()` 解析嵌入的 `agents/arguswing.toml` 并返回 `AgentRecord`。

**关键常量：**
- `DEFAULT_AGENT_ID`：`"arguswing"`，默认 agent 的稳定 ID

**初始化流程：**
1. `AppContext::init()` 调用 `ensure_default_agent()`
2. `ensure_default_agent()` 调用 `load_arguswing()` 并 upsert 到数据库
3. 消费者通过 `create_default_agent()` 或 `create_default_agent_with_approval()` 创建运行时 agent
