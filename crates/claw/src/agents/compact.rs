...
use crate::agents::thread::{CompactError, Thread};
use crate::llm::{ChatMessage, Role};

/// Compactor trait - responsible for deciding when and how to compact.
///
/// Implementations determine:
/// 1. Whether compaction is needed based on thread state
...
pub trait Compactor: Send + Sync {
    ...
    async fn compact(&self, thread: &mut Thread) -> Result<(), CompactError>;
...
impl Compactor for KeepRecentCompactor {
    async fn compact(&self, thread: &mut Thread) -> Result<(), CompactError> {
        let context_window = thread.provider().context_window();
        let threshold = (context_window as f32 * self.threshold_ratio) as u32;

        // Check if compaction is needed
        if thread.token_count() < threshold {
            return Ok(());
        }

        // Perform compaction
        let messages = thread.messages_mut();
        ...
        *messages = [system_msgs, recent].concat();
        ...
        // Update token count
        thread.recalculate_token_count();

        tracing::debug!(
            compactor = self.name(),
            new_token_count = thread.token_count(),
...
impl Compactor for KeepTokensCompactor {
    async fn compact(&self, thread: &mut Thread) -> Result<(), CompactError> {
        ...
        *messages = [system_msgs, kept].concat();
        ...
        // Update token count
        thread.set_token_count(current_tokens);
