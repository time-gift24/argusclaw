...
impl HookHandler for ApprovalHook {
    async fn on_tool_event(&self, ctx: &ToolHookContext) -> HookAction {
        ...
        let approval_keys = self.approval_keys(ctx);

        // First check runtime allow list (user has already approved this tool)
        if approval_keys.iter().any(|key| {
            self.runtime_allow
                .read()
        ...
        if !approval_keys
            .iter()
            .any(|key| self.policy.requires_approval(key))
        {
            return HookAction::Continue;
        }
...
mod tests {
    ...
    fn make_ctx(tool_name: &str, tool_input: serde_json::Value) -> ToolHookContext {
        ToolHookContext {
    ...
    async fn test_approval_hook_skips_tools_not_in_policy() {
        ...
        let ctx = make_ctx("read_file", serde_json::json!({"path": "/tmp/test.txt"}));
    ...
    async fn test_approval_hook_skips_tools_in_allow_list() {
        ...
        let ctx = make_ctx("shell", serde_json::json!({"command": "echo test"}));
