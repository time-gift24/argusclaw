...
/// Agent commands for interactive conversations.
#[derive(Debug, Parser)]
#[command(name = "agent", about = "Agent commands for interactive conversations")]
pub struct AgentCli {
    #[command(subcommand)]
...
/// Try to run an agent command if the first arg matches.
///
...
pub async fn try_run(ctx: AppContext) -> Result<bool> {
    ...
    let cli = AgentCli::try_parse_from(&args)?;
    run(ctx, cli.command).await?;
    Ok(true)
}

/// Run an agent command.
pub async fn run(ctx: AppContext, command: AgentCommand) -> Result<()> {
    match command {
        AgentCommand::Chat {
            provider,
            system_prompt,
