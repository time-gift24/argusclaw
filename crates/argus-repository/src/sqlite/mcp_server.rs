...
impl McpServerRepository for ArgusSqlite {
    async fn list(&self) -> Result<Vec<McpServerRecord>, DbError> {
        let rows = sqlx::query(
            r#"
            SELECT id, name, display_name, server_type, command, url, headers, use_sse, args, auth_token_ciphertext, auth_token_nonce, enabled
            FROM mcp_servers
    ...
    async fn create(&self, record: &McpServerRecord) -> Result<McpServerId, DbError> {
        ...
        sqlx::query(
            r#"
            INSERT INTO mcp_servers (name, display_name, server_type, command, url, headers, use_sse, args, auth_token_ciphertext, auth_token_nonce, enabled)
    ...
    async fn update(&self, record: &McpServerRecord) -> Result<(), DbError> {
        ...
        sqlx::query(
            ...
            SET name = ?2, display_name = ?3, server_type = ?4, command = ?5, url = ?6,
                headers = ?7, use_sse = ?8, args = ?9, auth_token_ciphertext = ?10, auth_token_nonce = ?11,
